...
import { LocalFolderPickerModal } from "@/components/feedback/local-folder-picker-modal";
import { RemoteEditUploadModal } from "@/components/feedback/remote-edit-upload-modal";
...
import {
  ...
  getFileModified,
  listenRemoteEditChanged,
  openInEditor,
  removeRemotePaths,
  renameRemoteFile,
  prepareRemoteEdit,
  setRemoteEditSessionPromptMode,
  startRemoteEditWatch,
...
import { useRemoteListing } from "@/hooks/use-remote-listing";
...
type RemoteEditDialogState = {
  ...
  onEnableAutoUploadForSession: () => void;
  onUploadDone: () => void;
};
...
export function FileBrowser({
  ...
  const [remoteEditDialogState, setRemoteEditDialogState] = useState<RemoteEditDialogState | null>(null);
  const [downloadPickerPaths, setDownloadPickerPaths] = useState<string[] | null>(null);
  ...
  const { items, setItems, isLoading, loadError, refreshFiles, loadRemoteFiles } = useRemoteListing({
    connectionId,
    path,
    onItemsChange,
    lastCompletedAt,
  });
  const inlineEditEntry = useInlineEditPreview(items, inlineEdit);
  const tableItems = useMemo(() => {
    if (inlineEdit?.kind === "rename" && inlineEdit.targetPath) {
      return items.filter((entry) => entry.path !== inlineEdit.targetPath);
    }
    return items;
  }, [inlineEdit, items]);
  const remoteEditUnlistenersRef = useRef<Array<() => void>>([]);
  const remoteEditTimersRef = useRef<number[]>([]);
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent) => {
      ...
      console.log("[D&D] Drop detected on remote panel:", connectionId, path);
      ...
      if (payload.sourceId === connectionId && payload.sourcePath === targetPath) return;

      const itemNames = payload.items.map((i) => i.name);
      const itemPaths = payload.items.map((i) => i.path);
      const isInternalRemoteMove = payload.sourceId === connectionId && payload.sourceId !== "local";
      const initLabel = itemNames.length === 1 ? itemNames[0] : `${itemNames.length} Dateien`;
      if (isInternalRemoteMove) {
        addLog("info", `PENDING: Initializing internal move for '${initLabel}'...`);
        pushToast("info", "Internal Move", `Initialisiere Move für ${initLabel}...`, 2200);
      } else {
      ...
      console.log("[D&D] Processing drop:", { sourceId: payload.sourceId, targetId: connectionId, targetPath, items: itemNames });

      void (async () => {
        try {
          const collisions = await checkCollisions(connectionId, targetPath, itemNames);
          if (collisions.length > 0) {
            setCollisionState({
              ...
              targetPath,
              mode: isInternalRemoteMove ? "internal_move" : "transfer",
            });
            return;
          }
          if (isInternalRemoteMove) {
            ...
            try {
              await enqueueInternalMove(payload.sourceId, movingPaths, targetPath);
            } finally {
              setRowStateByPath((prev) => {
                const next = { ...prev };
            ...
            await enqueueJob({
              ...
              target_session_id: connectionId,
    ...
    [connectionId, path, enqueueJob, enqueueInternalMove, pushToast, addLog],
  );
  ...
  const handleCollisionOverwrite = useCallback(() => {
    ...
    if (mode === "internal_move") {
      void (async () => {
        ...
        try {
          await enqueueInternalMove(sourceId, allItemPaths, targetPath);
        } finally {
          setRowStateByPath((prev) => {
            const next = { ...prev };
  ...
  }, [collisionState, connectionId, enqueueJob, enqueueInternalMove, pushToast]);
  ...
  const handleCollisionSkip = useCallback(() => {
    ...
    if (mode === "internal_move") {
      void (async () => {
        ...
        try {
          await enqueueInternalMove(sourceId, filtered, targetPath);
        } finally {
          setRowStateByPath((prev) => {
            const next = { ...prev };
  ...
  async function submitInlineEdit() {
    ...
    try {
      ...
      if (inlineEdit.kind === "rename" && inlineEdit.targetPath) {
        ...
        await renameRemoteFile(connectionId, inlineEdit.targetPath, focusPath);
      } else {
        focusPath = joinRemotePath(path, trimmed);
        if (inlineEdit.mode === "folder") {
          await createRemoteDirectory(connectionId, focusPath);
        } else {
          await createRemoteFile(connectionId, focusPath);
        }
      }
      const refreshed = await loadRemoteFiles();
      setItems(refreshed);
  ...
  async function confirmDelete() {
    ...
    try {
      await enqueueDeleteBatch(connectionId, paths.length, async () => {
        await removeRemotePaths(connectionId, paths, recursive);
      });
  ...
  function openEditorForFile(currentFile: FileEntry) {
    void prepareRemoteEdit(connectionId, currentFile.path)
      .then(async (session) => {
        const customEditor = settings?.editorMode === "custom" ? settings.customEditorPath : null;
        await openInEditor(session.local_path, customEditor);
        await startRemoteEditWatch(session.session_id, Boolean(settings?.autoUploadOnSave));
        let suppressPromptForSession = false;
        const unlisten = await listenRemoteEditChanged(async (event) => {
          if (event.session_id !== session.session_id) return;
          if (settings?.autoUploadOnSave || suppressPromptForSession) {
            await confirmRemoteEditUpload(session.session_id);
            refreshFiles(false);
            return;
          }
          setRemoteEditDialogState({
            sessionId: session.session_id,
            fileName: currentFile.name,
            previousSize: event.previous_size,
            currentSize: event.current_size,
            lastModified: event.last_modified,
            suppressPromptForSession: false,
            onEnableAutoUploadForSession: () => {
              suppressPromptForSession = true;
            },
            onUploadDone: () => {
              unlisten();
            },
        ...
        remoteEditUnlistenersRef.current.push(unlisten);
        const watchTimer = window.setInterval(async () => {
          const currentModified = await getFileModified(session.local_path).catch(() => session.last_modified);
          if (currentModified <= session.last_modified) return;
          window.clearInterval(watchTimer);
        }, 1200);
        remoteEditTimersRef.current.push(watchTimer);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    return () => {
      for (const timer of remoteEditTimersRef.current) {
        window.clearInterval(timer);
      }
      remoteEditTimersRef.current = [];
      for (const unlisten of remoteEditUnlistenersRef.current) {
        unlisten();
      }
      remoteEditUnlistenersRef.current = [];
    };
  }, []);
  ...
  return (
    <section
      ...
      data-drop-session-id={connectionId}
      data-drop-path={path}
      className={[
    ...
    >
      ...
      {menu ? (
        <ContextMenu
          ...
          transferActionLabel="Herunterladen"
          ...
          onChmod={() => {
            ...
            void chmodRemoteFile(connectionId, file.path, mode)
              .then(() => {
                pushToast("success", `chmod gesetzt: ${file.name}`);
                return loadRemoteFiles().then(setItems);
              })
      ...
      <LocalFolderPickerModal
        open={downloadPickerPaths !== null}
        onCancel={() => setDownloadPickerPaths(null)}
        onSelect={(folder) => {
          const paths = downloadPickerPaths;
          setDownloadPickerPaths(null);
          if (!paths?.length) return;
          void enqueueJob({
            source_session_id: connectionId,
            target_session_id: "local",
            selected_items: paths,
            target_path: folder,
          }).catch((error) => {
            const message = error instanceof Error ? error.message : String(error);
            pushToast("error", "Download fehlgeschlagen", message, 5000);
          });
        }}
      />
      <RemoteEditUploadModal
        open={Boolean(remoteEditDialogState)}
        fileName={remoteEditDialogState?.fileName ?? ""}
        previousSize={remoteEditDialogState?.previousSize ?? 0}
        currentSize={remoteEditDialogState?.currentSize ?? 0}
        lastModified={remoteEditDialogState?.lastModified ?? 0}
        suppressPromptForSession={remoteEditDialogState?.suppressPromptForSession ?? false}
        onSuppressPromptForSessionChange={(value) =>
          setRemoteEditDialogState((current) => (current ? { ...current, suppressPromptForSession: value } : current))
        }
        onDiscard={() => {
          void (async () => {
            const dialogState = remoteEditDialogState;
            if (!dialogState) return;
            if (dialogState.suppressPromptForSession) {
              await setRemoteEditSessionPromptMode(dialogState.sessionId, true);
              dialogState.onEnableAutoUploadForSession();
            }
            setRemoteEditDialogState(null);
          })().catch((error) =>
            pushToast("error", error instanceof Error ? error.message : "Upload-Einstellungen konnten nicht gespeichert werden"),
          );
        }}
        onUpload={() => {
          void (async () => {
            ...
            await confirmRemoteEditUpload(dialogState.sessionId);
            refreshFiles(false);
            dialogState.onUploadDone();
            setRemoteEditDialogState(null);
            pushToast("success", "Änderung wurde hochgeladen.");
          })().catch((error) =>
            pushToast("error", error instanceof Error ? error.message : "Upload fehlgeschlagen"),
          );
        }}
      />
      {collisionState ? (
